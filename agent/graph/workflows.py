...
from agent.graph.nodes import (error_handling_node, file_downloading_node,
                               file_questions_node, file_selection_node,
                               files_getting_node)
...
def router(state, current_node):
    ...
    match current_node:
        case "files_getting":
            return (
                "file_selection"
                if state.get("available_files", None) is not None
                else "__end__"
            )
        case "file_selection":
            return (
                "file_downloading"
                if state.get("selected_file_id", False)
                else "__end__"
            )
        case "file_downloading":
            return (
                "file_questions"
                if state.get("selected_file_path", False)
                else "__end__"
            )
        case "file_questions":
...
workflow.add_conditional_edges("__start__", start_router)
workflow.add_conditional_edges(
    "files_getting", lambda state: router(state, "files_getting")
)
workflow.add_conditional_edges(
    "file_selection", lambda state: router(state, "file_selection")
)
workflow.add_conditional_edges(
    "file_downloading", lambda state: router(state, "file_downloading")
)
workflow.add_conditional_edges(
    "file_questions", lambda state: router(state, "file_questions")
)
...
async def interact(user_id: str, messages: list[BaseMessage]) -> str:
    ...
    if saved_state.values:
        ...
        state = State(
            user_id=user_id, message_history=messages, error=None, error_type=None
        )
